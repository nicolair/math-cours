...
\debutcours{Notes sur \emph{Measure an Integration} de König}{beta 1. 9\tiny{ le \today} }
...
\subsubsection{Unions de classes de congruences} \label{ExpleZ}
Ici $X = \Z$. \index{Exemple arithmétique}
L'ensembles des unions finies de classes de congruence défini au dessous sera désigné dans tout le document par \og Exemple arithmétique\fg.
...
\subsubsection{Union de cubes dyadiques}
\begin{defi}[s-cube]\index{s-cube}\label{s-cube}
Pour tout naturel non nul $s$, on appelle $s$-cube un cube fermé de côté $\frac{1}{2^s}$ et de sommets à coordonnées dans $\frac{1}{2^s}\Z$.\newline
...
\end{displaymath}
On note $\mathfrak{M}_s$ l'ensemble des $s$-cubes, $\mathfrak{S}_s$ l'ensemble des unions finies de $s$-cubes et $\mathfrak{S} = \bigcup_{s\in \N} \mathfrak{S}_s$.
\end{defi}

\begin{rems}
 \begin{enumerate}
  \item Un $0$-cube est un cube de côté $1$ et de sommets à coordonnées entières.  Pour tout $s$-cube $Q$, il existe un unique $a\in \frac{1}{2^s}\Z$. De plus $X = \R^n = \bigcup_{a\in \frac{1}{2^s}\Z}Q_a$.
  \item La décomposition d'un élément de $\mathfrak{M_s}$ comme union de $s$-cubes est unique. En particulier chaque $s$-cube est la réunion de $2^n$ cubes dans $\mathfrak{S}_{s+1}$. On en déduit que $\mathfrak{S}_s \subset \mathfrak{S}_{s+1}$.
  ...
  Pour $A$ et $B$ dans $\mathfrak{S}$, il existe $s\in \N$ tels que $A$ et $B$ soient des unions d'ensembles $\mathfrak{A}$, $\mathfrak{B}$ de $s$-cubes alors:
    \begin{displaymath}
      \left.
      \begin{aligned}
        A = \bigcup_{Q\in \mathfrak{A}} Q \\
        B = \bigcup_{Q\in \mathfrak{B}} Q,
      \end{aligned}
      \right \rbrace
      \Rightarrow
      \left\lbrace
      \begin{aligned}
        A\cup B = \bigcup_{Q\in \mathfrak{A}\cup\mathfrak{B}} Q \\
        \mathrm{adherence}(A\cap B) = \bigcup_{Q\in \mathfrak{A}\cap\mathfrak{B}} Q \\
        \mathrm{adherence}(A\setminus B) = \bigcup_{Q\in \mathfrak{A}\setminus\mathfrak{B}} Q
      \end{aligned}
      \right. .
    \end{displaymath}
   \item On pourrait récupérer la stablité pour l'intersection et la soustraction en considérant des cubes ouverts au lieu de fermés mais alors aucun point à coordonnées dans $\frac{1}{2^s}\Z$ ne serait dans un $s$-cube et on perdrait $\mathfrak{S}_{s+1} \subset \mathfrak{S}_s$.
 \end{enumerate}
\end{rems}

On définit le volume dans $\mathfrak{S}$ à partir du cube unité, de l'invariance par translation, du caractère modulaire du volume comme fonction d'ensemble et de l'unique décomposition .
\begin{itemize}
...
\begin{defi}[volume dans l'ensemble des unions de cubes dyadiques]
...
\end{demo}
\begin{propn}
 La fonction d'ensembles $\lambda$ (volume) définie de $\mathfrak{S}$ dans $[0,+\infty[$ est isotone (croissante) et additive.
\end{propn}
\begin{demo}
 C'est une conséquence de la définition à partir du nombre de $s$-cubes dans une décomosition.
\end{demo}
Elle n'est pas modulaire à cause de la non stabilité pour l'intersection.

\subsubsection{Volume d'une partie bornée ou compacte}
\noindent On note $\mathfrak{B}$ l'ensemble des parties bornées  de $\R^n$ et $\mathfrak{K} \subset \mathfrak{B}$ l'ensemble des parties compactes. On étend le volume $\lambda$ à $\mathfrak{B}$ et $\mathfrak{K}$.
...
\end{displaymath}
\index{système de parties!treillis} \index{système de parties!ovale} \index{système de parties!anneau} \index{système de parties!algèbre}
\begin{defi}
...
\begin{displaymath}
 \forall n \in \N, \; S_n = \bigcup_{k =0}^n \left(a_k + \mu^{k+p} \Z\right).
\end{displaymath}
